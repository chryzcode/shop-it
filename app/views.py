...
def edit_draft_newsletter(request, pk):
    ...
    if Subscription_Timeline.objects.filter(store=store).exists():
        ...
        messages.error(request, "You have subscribe to a plan")
        return redirect("app:newsletter_page")
    return render(request, "store/all-newsletter-page.html", {"store": store, "newsletter": newsletter, "form": form, "subscribers": subscribers, "store_newsletter": store_newsletter})
    ...
    if request.user.is_superuser:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                company = form.cleaned_data["shipping_company"]
    ...
    if request.user.is_superuser:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                shipping_method = form.save(commit=False)
                if Shipping_Method.objects.filter(
                    state=shipping_method.state, country=shipping_method.country, location= shipping_method.location
                ).exists():
                    error = "Shipping Method already exists"
                    return render(
                        request,
